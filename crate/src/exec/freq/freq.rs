...
pub fn freq_of_memory(
    ...
    // max/min are None on an empty batch (e.g. the current event has no
    // samples); report a zeroed extent instead of panicking
    let max_mem = arrow::compute::max(get_int32_column(&batch, 2)).unwrap_or(0);
    let min_mem = arrow::compute::min(get_int32_column(&batch, 2)).unwrap_or(0);
    let max_time = arrow::compute::max(get_floatarray_column(&batch, 0)).unwrap_or(0.);
    let min_time = arrow::compute::min(get_floatarray_column(&batch, 0)).unwrap_or(0.);
    let max_freq = arrow::compute::max(get_floatarray_column(&batch, 3)).unwrap_or(0.);
    let min_freq = arrow::compute::min(get_floatarray_column(&batch, 3)).unwrap_or(0.);
