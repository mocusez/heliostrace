...
pub fn get_top_srclines(record_batch: RecordBatch, ordered_by: usize) -> RecordBatch {
    ...
    // Setting the global top five (may be fewer than 5 rows, e.g. traces with
    // an empty uir dictionary such as Metal/interpreter-only runs)
    let global_top_five = get_max_top_five(srcline_batch_sorted_after_coverage.clone());
    let global_top_five_len = global_top_five.num_rows();
    top_five.push(global_top_five);
    for _i in 0..global_top_five_len {
    ...
    for entry in op.into_iter().enumerate() {
        if entry.0 < global_top_five_len {
            op_vec.push("root");
        } else {
