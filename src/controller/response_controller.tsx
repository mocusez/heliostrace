...
export function setHeliosFileReadingFinished() {
    store.dispatch({
...
//set Metal GPU dispatches (optional: empty when the trace has no metal.json) in redux store
export function setMetalDispatches(metalDispatches: Array<model.IMetalDispatch>) {
    store.dispatch({
        type: model.StateMutationType.SET_METAL_DISPATCHES,
        data: metalDispatches,
    });
}

//set analytic roofline data (optional: undefined when the trace has no roofline.json) in redux store
export function setRoofline(roofline: model.IRooflineData | undefined) {
    store.dispatch({
        type: model.StateMutationType.SET_ROOFLINE,
        data: roofline,
    });
}
...
function storeChartDataFromRust(requestId: number, resultObject: model.IResult, requestType: model.BackendQueryType) {
    ...
    const chartDataCollection: model.IChartDataKeyValue = store.getState().chartData;
    ...
    // dispatch a fresh object: reusing the mutated collection keeps the same
    // reference, so connected components never see the chartData prop change
    store.dispatch({
