...
export function requestChartData(controller: RequestController, chartId: number, chartType: model.ChartType) {
    ...
    switch (chartType) {
        ...
        case model.ChartType.UIR_VIEWER:
            ...
            restQuery = model.createBackendQuery({
                type: restQueryType,
                data: { events: store.getState().events!, pipelines: store.getState().currentPipeline, operators: store.getState().currentOperator, timeBucketFrame: store.getState().currentTimeBucketSelectionTuple },
            });
            break;
