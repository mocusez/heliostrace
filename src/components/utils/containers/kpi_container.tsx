...
class KpiContainer extends React.Component<Props> {
    ...
    createKpiCard(key: number, title: string, valueElement: JSX.Element, isUnfedined?: boolean) {
        return <div key={key} className="mx-[4px] flex h-[55px] w-[150px] basis-[100px] flex-col items-center justify-center rounded-[70px] border-[0.5px] p-[5px] max-[900px]:my-[5px]" >
            <div>
                <p className="m-0 max-w-[80px] text-[11px]" style={{ color: this.props.appContext.tertiaryColor }}>
                    {title}
                </p>
    ...
    public render() {
        ...
        return <div className="flex h-full flex-row items-center justify-center text-center">
            {this.isComponentLoading() ?
                <Spinner />
                : <div className="flex w-full flex-row flex-nowrap justify-center max-[900px]:flex-wrap max-[900px]:items-center">
