...
import { THEME_CHANGE_EVENT } from '../../style/theme';
...
class ChartWrapper extends React.Component<Props, State> {
    ...
    shouldComponentUpdate(nextProps: Props, nextState: State) {
        ...
        //rerender on light/dark theme change so vega specs re-read ink colors
        if (this.state.themeVersion !== nextState.themeVersion) {
            return true;
        }
    ...
    createChildChart() {
        ...
        const partialChartProps: model.ICommonChartProps = {
            key: `${this.state.chartId}-${this.state.width}-${this.state.height}-${this.state.themeVersion}`,
    ...
    renderChartOptions(): JSX.Element | undefined {
        ...
        return chartOptions ? <div className="flex flex-wrap items-center justify-center pb-[10px]">
            {chartOptions}
        </div> : undefined;
    }
    ...
    public render() {
        ...
        // TMAM data is optional (traces without tmam.csv yield an empty batch);
        // hide the whole swim lane when there is nothing to show.
        if (this.props.chartType === model.ChartType.SWIM_LANES_TMAM && !this.isChartDataLoading()) {
            const tmamData = this.props.chartData[this.state.chartId]?.chartData?.data as model.ISwimlanesTmamData | undefined;
            if (!tmamData || !tmamData.buckets || tmamData.buckets.length === 0) {
                return null;
            }
        }

        return <div className="flex h-full w-full flex-col items-center justify-center" ref={this.elementWrapper}>
            {this.renderChartOptions()}
            {this.isChartDataLoading()
                ? <Spinner />
                : <div className="flex h-full w-full justify-center overflow-visible animate-in fade-in">
